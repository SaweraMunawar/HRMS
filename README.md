...
| Needed | Why |
|---|---|
| Node.js 20 or newer | Next.js 16 requires it |
| npm | Ships with Node |
| A working internet connection during `npm install` | `better-sqlite3` downloads a prebuilt binary for your platform |

`better-sqlite3` is a native module. It normally installs a ready-made binary and needs no compiler. If that download is blocked — a proxy, a firewall or a dropped connection — npm falls back to building it from source, which then needs platform build tools:
...
- **Linux:** `build-essential` and Python 3

Installing those tools is only necessary if the prebuilt download fails. See [Troubleshooting](#troubleshooting) if it does.
...
**`npm install` fails on `better-sqlite3` with `gyp ERR! find VS`**

The prebuilt binary could not be downloaded, so npm tried to compile it and could not find a C++ toolchain. Two ways out:

1. Clear the npm cache and retry — this is often a transient network failure:
```bash
   npm cache clean --force
   npm install
```
2. If it keeps failing, install the build tools listed in [Before you start](#before-you-start) and run `npm install` again. On Windows, "Visual Studio C++ core features" alone is not enough; the **Desktop development with C++** workload must be selected.

**`Error: Cannot find module '@/generated/prisma/client'`**

The Prisma client was not generated. Run:
```bash
npx prisma generate
```

**Login does nothing, or the session drops immediately**

`AUTH_SECRET` in `.env` is missing or still the placeholder. Generate one with the command in [Environment variables](#environment-variables) and restart the dev server.
